...
def val(model, dataloader):
    ...
    li_pred = []
    li_gt = []
    for i, (_, im, _, lb) in enumerate(dataloader):
        im, lb = Variable(im), Variable(lb)
        im, lb = im.float().cuda(), lb.long().cuda()
        pred = model(im)
        # Mean IoU
...
def train():
    ...
    for epoch in range(config.max_epoch):
        for i, (_, im, _, lb) in enumerate(train_dataloader):
            im, lb = Variable(im), Variable(lb)
            im, lb = im.float().cuda(), lb.long().cuda()
            optimizer.zero_grad()
            # forward
...
def eval_on_validation():
    ...
    li_gt = []
    total_time = 0
    for i, (name, im, _, lb) in enumerate(test_dataloader):
        im, lb = Variable(im), Variable(lb)
        im, lb = im.float().cuda(), lb.long().cuda()
        start = time.clock()
...
if __name__ == '__main__':
    ...
    if config.split is 'train':
        ...
        config.model_prefix = 'checkpoints/best-model-fcn8s_rgb@'
        config.batch_size = 4
        ...
        config.model = 'FCN8s'
        config.load_model_path = 'checkpoints/best-model-fcn8s_rgb@K_0.20.pth'
        config.run_dir = 'RUN/val_fcn8s_rgb'
        # eval_on_validation()
        eval_on_validation()
